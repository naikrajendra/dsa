 ...
 * chaotic if someone has bribed more than people.
 * Input Format
 * 
...
public class MinimumBribes {
    ...
    public static void minimumBribes(List<Integer> q) {
        ...
        for (int i = 0; i < q.size(); i++) {
            // The person's original position (1-indexed)
            int originalPosition = q.get(i);
            // Their current position in the array (0-indexed, so add 1)
            int currentPosition = i + 1;

            // Check if this person moved forward by more than 2 spots
            ...
            // A person could only have been bribed by people who started ahead of them.
            // Due to the "at most 2 bribes" rule, the furthest back a brier could have
            // been pushed is one spot ahead of the victim's original position
            // (originalPosition - 2).
            int startLookIndex = Math.max(0, originalPosition - 2);
            for (int j = startLookIndex; j < i; j++) {
    ...
    public static void main(String[] args) {
        ...
        minimumBribes(q2); // Expected output: "Too chaotic"
    }
}
